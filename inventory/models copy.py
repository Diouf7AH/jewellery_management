...
class InventoryMovement(models.Model):
    ...
    def clean(self):
        ...
        if self.movement_type == MovementType.SALE_OUT:
            exists = InventoryMovement.objects.select_for_update().filter(
                vente_ligne=self.vente_ligne,
                produit_line=self.produit_line,
                movement_type=MovementType.SALE_OUT
            ).exclude(pk=self.pk).exists()

            if exists:
                raise ValidationError({
                    "non_field_errors": "Ce mouvement SALE_OUT existe déjà pour cette ligne et ce produit (FIFO)."
                })
