...
# # --- helpers temps (zoneinfo-safe) ---
# def _aware(dt: datetime) -> datetime:
#     return timezone.make_aware(dt) if timezone.is_naive(dt) else dt

# def _year_bounds(y: int):
#     start = _aware(datetime(y, 1, 1, 0, 0, 0))
#     end   = _aware(datetime(y + 1, 1, 1, 0, 0, 0))   # exclusif
#     return start, end
...
#     - Sans paramètres: récap par type **pour l’année en cours**
#     - Périmètre:
#         admin  : tout
#         manager: mouvements de SA bijouterie (src ou dst)
#         vendor : SES mouvements
#     """
...
#             qs = (InventoryMovement.objects
#                   .select_related("src_bijouterie", "dst_bijouterie", "vendor", "vendor__user", "produit"))
...
#                 if not bj_id:
#                     return Response({"period": None, "scope": scope, "results": []}, status=200)
#                 qs = qs.filter(Q(src_bijouterie_id=bj_id) | Q(dst_bijouterie_id=bj_id))
#             elif role == ROLE_VENDOR:
#                 scope = "vendor"
#                 ven = Vendor.objects.filter(user=request.user).first()
#                 if not ven:
#                     return Response({"period": None, "scope": scope, "results": []}, status=200)
#                 qs = qs.filter(vendor_id=ven.id)
#             else:
#                 return Response({"error": "Accès refusé"}, status=status.HTTP_403_FORBIDDEN)

#             # --- Fenêtre: année en cours (défaut demandé) ---
#             year = timezone.localdate().year
#             start, end = _year_bounds(year)
#             qs = qs.filter(occurred_at__gte=start, occurred_at__lt=end)

#             # --- Agrégations ---
#             safe_unit = Coalesce(F("unit_cost"), Decimal("0.00"))
#             total_value_expr = ExpressionWrapper(
#                 F("qty") * safe_unit,
#                 output_field=DecimalField(max_digits=18, decimal_places=2)
#             )

#             raw = (qs.values("movement_type")
#                      .annotate(
#                          count=Count("id"),
#                          total_qty=Coalesce(Sum("qty"), 0),
#                          total_value=Coalesce(Sum(total_value_expr), Decimal("0.00")),
#                      ))

#             # On veut TOUTES les catégories, même 0
#             by_type = {row["movement_type"]: row for row in raw}
#             labels = dict(MovementType.choices)
#             results = []
#             for code, label in MovementType.choices:
...
#                     "total_value": f"{Decimal(row['total_value']):.2f}" if row else "0.00",
#                 })

#             return Response({
#                 "period": {
#                     "mode": "year",
#                     "start": start.astimezone(py_tz.utc).isoformat().replace("+00:00","Z"),
#                     "end":   end.astimezone(py_tz.utc).isoformat().replace("+00:00","Z"),
#                 },
#                 "scope": scope,
#                 "results": results
#             }, status=200)
...
#             # Pendant le dev, on surface l’erreur pour éviter un 500 silencieux
#             return Response({"detail": "Erreur inattendue", "error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
# <-- importe ton serializer de réponse (déjà défini)
from .serializers import InventoryMovementResponseSerializer


# --- helpers temps (compatibles zoneinfo) ---
def _aware(dt: datetime) -> datetime:
    return timezone.make_aware(dt) if timezone.is_naive(dt) else dt
...
class InventoryMovementListView(APIView):
    ...
    @swagger_auto_schema(
        operation_summary="Récapitulatif par type (année en cours) + stocks par bijouterie et par vendeur",
    ...
    def get(self, request):
        try:
            ...
            if role == ROLE_ADMIN:
                ...
                bj_id = getattr(getattr(mgr, "bijouterie", None), "id", None)
                if not bj_id:
                    # Pas de bijouterie attachée → retourne des totaux vides
                    return Response({
                        "period": None, "scope": scope,
                        "results": [], "stock_by_bijouterie": [], "stock_by_vendor": []
                    }, status=200)
                qs = qs.filter(Q(src_bijouterie_id=bj_id) | Q(dst_bijouterie_id=bj_id))

            elif role == ROLE_VENDOR:
                scope = "vendor"
                ven = Vendor.objects.filter(user=request.user).select_related("bijouterie", "user").first()
                if not ven:
                    return Response({
                        "period": None, "scope": scope,
                        "results": [], "stock_by_bijouterie": [], "stock_by_vendor": []
            ...
            # ---- Fenêtre: année en cours ----
            year = timezone.localdate().year
            start, end = _year_bounds(year)
            qs = qs.filter(occurred_at__gte=start, occurred_at__lt=end)

            # ---- Agrégations mouvements (par type) ----
            safe_unit = Coalesce(F("unit_cost"), Decimal("0.00"))
            total_value_expr = ExpressionWrapper(
                F("qty") * safe_unit,
                output_field=DecimalField(max_digits=18, decimal_places=2)
            ...
            # Inclure tous les types même à 0
            by_type = {row["movement_type"]: row for row in raw}
            labels = dict(MovementType.choices)
            results = []
            for code, label in MovementType.choices:
            ...
            # ----- Snapshot stock par bijouterie -----
            if scope == "admin":
                qs_bj = Stock.objects.select_related("bijouterie")
            elif scope == "manager":
                qs_bj = Stock.objects.select_related("bijouterie").filter(bijouterie_id=bj_id)
            else:  # vendor
                bj_vendor_id = getattr(getattr(ven, "bijouterie", None), "id", None)
                qs_bj = (Stock.objects.select_related("bijouterie")
                         .filter(bijouterie_id=bj_vendor_id)) if bj_vendor_id else Stock.objects.none()

            agg_bj = (qs_bj.values("bijouterie_id", "bijouterie__nom")
                           .annotate(
                               quantite_allouee=Coalesce(Sum("quantite_allouee"), 0),
                               quantite_disponible=Coalesce(Sum("quantite_disponible"), 0),
                           )
            ...
            stock_by_bijouterie = [
                {
                    ...
                    "quantite_allouee": row["quantite_allouee"],
                    "quantite_disponible": row["quantite_disponible"],
                }
                for row in agg_bj
            ]

            # ----- Snapshot stock par vendeur -----
            if scope == "admin":
                qs_v = VendorStock.objects.select_related("vendor__user", "vendor__bijouterie")
            elif scope == "manager":
                qs_v = (VendorStock.objects.select_related("vendor__user", "vendor__bijouterie")
                        .filter(vendor__bijouterie_id=bj_id))
            else:  # vendor
                qs_v = (VendorStock.objects.select_related("vendor__user", "vendor__bijouterie")
                        .filter(vendor_id=ven.id))

            agg_v = (qs_v.values(
                        "vendor_id",
                        "vendor__user__email",
                        "vendor__bijouterie_id",
                    ...
                    .order_by("vendor__bijouterie__nom", "vendor__user__email"))

            stock_by_vendor = [
                {
                    "vendor_id": row["vendor_id"],
                    "vendor_email": row["vendor__user__email"],
                    "bijouterie_id": row["vendor__bijouterie_id"],
                    "bijouterie_nom": row["vendor__bijouterie__nom"],
                    "quantite_allouee": row["quantite_allouee"],
                    "quantite_disponible": row["quantite_disponible"],
                }
                for row in agg_v
            ...
            return Response({
                ...
                "scope": scope,
                "results": results,
                "stock_by_bijouterie": stock_by_bijouterie,
                "stock_by_vendor": stock_by_vendor,
            }, status=200)

        except Exception as e:
            # Surface l’erreur en dev pour éviter un 500 silencieux
            return Response({"detail": "Erreur inattendue", "error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
