...
from store.models import (Bijouterie, Categorie, Gallery,
...
class MarqueCreateSerializer(serializers.Serializer):
...
class CategorieWithModelesSerializer(serializers.Serializer):
    nom = serializers.CharField(
        max_length=30,
        help_text="Nom exact de la catégorie existante"
    )
    modeles = serializers.ListField(
        child=serializers.CharField(max_length=55),
        help_text="Liste de noms de modèles à créer et lier"
    )

    def validate_modeles(self, value):
        clean_modeles = [nom.strip().title() for nom in value if nom.strip()]
        if not clean_modeles:
            raise serializers.ValidationError("La liste des modèles ne peut pas être vide.")
        return clean_modeles
...
#Lister le modele
#exemple Si un modèle "Jonc" est lié à 2 catégories "Bague" et "Bracelet" :
#on liste le modele "Jonc" et ces catégories
class CategorieSimpleSerializer(serializers.Serializer):
    id = serializers.IntegerField(source='categorie.id')
    nom = serializers.CharField(source='categorie.nom')
...
class ModeleListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Modele
        fields = ['id', 'modele', 'categories']

    def get_categories(self, obj):
        # Récupère toutes les liaisons Modele ↔ Catégorie
        liaisons = obj.modele_categories.select_related('categorie')
        return CategorieSimpleSerializer(liaisons, many=True).data
#end list modele
