...
class RachatClientItem(models.Model):
    ...
    movement = models.OneToOneField("stock_matiere_premiere.MatierePremiereMovement",on_delete=models.PROTECT,
                null=True,blank=True,related_name="rachat_item",)
    ...
    def __str__(self):
        return (
            f"{self.rachat.numero_ticket} - "
            f"{self.get_matiere_display()} - "
            f"{self.purete} - "
            f"{self.poids} g"
        )
...
class MatierePremiereStock(models.Model):
    ...
    def __str__(self):
        return (
            ...
            f"{self.get_matiere_display()} - "
            f"{self.purete} : {self.poids_total} g"
        )
...
class AchatMatierePremiereItem(models.Model):
    ...
    purete = models.ForeignKey(
        "store.Purete",
        on_delete=models.PROTECT,
        related_name="achats_matiere_items",
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["matiere"]),
            models.Index(fields=["purete"]),
            models.Index(fields=["matiere", "purete"]),
        ]

    def __str__(self):
        return f"{self.get_matiere_display()} - {self.purete} - {self.poids} g"
...
class Raffinage(models.Model):
    ...
    purete_avant = models.ForeignKey(
        "store.Purete",
        on_delete=models.PROTECT,
        related_name="raffinages_avant",
    )

    purete_apres = models.ForeignKey(
        "store.Purete",
        on_delete=models.PROTECT,
        related_name="raffinages_apres",
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["matiere"]),
            models.Index(fields=["created_at"]),
            models.Index(fields=["bijouterie", "matiere"]),
            models.Index(fields=["purete_avant"]),
            models.Index(fields=["purete_apres"]),
        ]

        constraints = [
            models.CheckConstraint(
                check=Q(perte__gte=0),
                name="ck_raffinage_perte_positive",
            ),
        ]

    def __str__(self):
        return (
            f"{self.numero_operation} - "
            f"{self.get_matiere_display()} - "
            f"{self.poids_entree} g → {self.poids_sortie} g"
        )
...
class StockRaffine(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                check=Q(poids_total__gte=0),
                name="ck_stock_raffine_positive",
            ),
        ]
...
class VenteMatierePremiere(models.Model):
    ...
    bijouterie = models.ForeignKey(
        "store.Bijouterie",
        on_delete=models.PROTECT,
        related_name="ventes_matiere_premiere",
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["created_at"]),
            models.Index(fields=["bijouterie", "matiere"]),
            models.Index(fields=["bijouterie", "created_at"]),
        ]
