...
def build_escpos_recu_paiement_80mm(
    *,
    ...
    date_paiement = _normalize_datetime(
        date_paiement
    )
    ...
    date_txt = date_paiement.strftime(
        "%d/%m/%Y"
    )

    heure_txt = date_paiement.strftime(
        "%H:%M"
    )
    ...
    # 4 lignes de marge après le message final
    out += b"\n\n\n\n"

    # PAS DE CUT :
    # cette imprimante ne reconnait pas GS V
    # et imprime le caractère "V"
