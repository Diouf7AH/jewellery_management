...
class Achat(models.Model):
    ...
    def update_total(self, save: bool = True):
        ...
        Calcule :
          base_HT = Σ (ligne.quantite * produit.poids * ligne.prix_achat_gramme)
          HT      = base_HT + frais_transport + frais_douane
          TAX     = 0 (ou somme de taxes si tu ajoutes ça plus tard)
          TTC     = HT + TAX
        """
        from purchase.models import \
            ProduitLine  # import local pour éviter les cycles

        expr_ht = ExpressionWrapper(
            F("quantite") * Coalesce(F("produit__poids"), 1) * Coalesce(F("prix_achat_gramme"), Decimal("0.00")),
        ...
        total_tax = Decimal("0.00")  # à adapter si tu ajoutes des taxes
