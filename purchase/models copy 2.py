...
#     · Les totaux sont calculés à partir des ProduitLine rattachées à l'achat :
#       base_HT = Σ (ligne.quantite * produit.poids * ligne.prix_achat_gramme)
#       HT      = base_HT + frais_transport + frais_douane
#       TAX     = 0 (sauf si tu rajoutes des taxes plus tard)
#       TTC     = HT + TAX
...
#     # ----------------- Totaux basés sur les ProduitLine -----------------
#     def update_total(self, save: bool = True):
#         """
#         Calcule :
#           base_HT = Σ (ligne.quantite * produit.poids * ligne.prix_achat_gramme)
#           HT      = base_HT + frais_transport + frais_douane
#           TAX     = 0 (pour l’instant)
#           TTC     = HT + TAX
#         """
#         from purchase.models import ProduitLine  # éviter les cycles d'import

#         total_ht = Decimal("0.00")

#         # On récupère toutes les lignes liées à cet achat via les lots
#         lignes = (
#             ProduitLine.objects
...
#         for pl in lignes:
#             # On ne comptabilise que si on a un prix et un poids
#             if pl.prix_achat_gramme and pl.produit.poids:
#                 qte   = Decimal(pl.quantite or 0)
#                 poids = Decimal(pl.produit.poids or 0)
#                 prixg = Decimal(pl.prix_achat_gramme)
#                 # quantite * poids * prix_achat_gramme
#                 total_ht += qte * poids * prixg

#         frais_transport = Decimal(self.frais_transport or 0)
#         frais_douane    = Decimal(self.frais_douane or 0)

#         self.montant_total_ht  = total_ht + frais_transport + frais_douane
#         self.montant_total_ttc = self.montant_total_ht  # TAX = 0 pour l’instant
...
class ProduitLine(models.Model):
    ...
    On stocke la quantité achetée. Les POIDS se déduisent : quantite × produit.poids.
    La quantité réellement disponible est gérée dans le module de stock.
    """
    ...
    def poids_total_calc(self):
        # quantité achetée × poids unitaire courant du produit
