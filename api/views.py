...
class ManagerDashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        for row in top_products_qs:
            produit_nom = " ".join(
                filter(
                    None,
                    [
                        row.get("produit__marque__marque"),
                        row.get("produit__modele__modele"),
                        row.get("produit__purete__purete"),
                    ],
                )
            )
        ...
        stock_summary = stock_qs.aggregate(
            ...
            total_quantite_totale=Coalesce(Sum("quantite_totale"), 0),
        ...
        stock_by_product_qs = (
            ...
            .values(
                ...
                "produit_line__produit__marque__marque",
                "produit_line__produit__modele__modele",
                "produit_line__produit__purete__purete",
            )
            .annotate(
                total_en_stock=Coalesce(Sum("en_stock"), 0),
                total_quantite_totale=Coalesce(Sum("quantite_totale"), 0),
        ...
        for row in stock_by_product_qs:
            produit_nom = " ".join(
                filter(
                    None,
                    [
                        row.get("produit_line__produit__marque__marque"),
                        row.get("produit_line__produit__modele__modele"),
                        row.get("produit_line__produit__purete__purete"),
                    ],
                )
            )
            ...
            stock_by_product.append(
                {
                    ...
                    "quantite_totale": int(row["total_quantite_totale"] or 0),
                }
            )
        ...
        return Response(
            {
                ...
                "stock_summary": {
                    ...
                    "total_quantite_totale": int(
                        stock_summary["total_quantite_totale"] or 0
                    ),
