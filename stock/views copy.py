...
from .serializers import (BijouterieReceiveInSerializer,
                          BijouterieToVendorInSerializer,
                          ReserveToBijouterieInSerializer, StockSerializer,
                          VendorAssignInSerializer)
from .services import (receive_bijouterie_stock_by_produit,
...
from backend.permissions import (ROLE_ADMIN, ROLE_MANAGER, ROLE_VENDOR,
                                 IsAdminOrManagerOrSelfVendor, get_role_name)
from backend.utils_role import (get_manager_bijouterie_id,
...
class BijouterieReceiveView(APIView):
    """
    POST /api/stocks/receive/bijouterie/

    Payload:
    {
      "bijouterie_id": 1,
      "lignes": [{"produit_id": 10, "quantite": 5}],
      "note": "Réception livraison"
    }
    """
    permission_classes = [IsAuthenticated, IsAdminOrManagerOrSelfVendor]
    ...
    @swagger_auto_schema(
        operation_id="receiveBijouterieStock",
        operation_summary="Réception boutique (allouée → disponible) (FIFO par produit)",
        operation_description=dedent("""
            Étape ERP de réception boutique:
            - diminue `quantite_allouee`
            - augmente `quantite_disponible`
            FIFO sur les ProduitLine du produit.
        """),
        request_body=BijouterieReceiveInSerializer,
        responses={
            200: openapi.Response("Résumé réception"),
    ...
    def post(self, request):
        s = BijouterieReceiveInSerializer(data=request.data)
        s.is_valid(raise_exception=True)

        try:
            res = receive_bijouterie_stock_by_produit(
                bijouterie_id=s.validated_data["bijouterie_id"],
