...
def get_role_name(user) -> Optional[str]:
    ...
    mp = getattr(user, "staff_manager_profile", None)
    if _is_verified_profile(mp):
        return ROLE_MANAGER

    cp = getattr(user, "staff_cashier_profile", None)
    if _is_verified_profile(cp):
        return ROLE_CASHIER

    vp = getattr(user, "staff_vendor_profile", None)
    if _is_verified_profile(vp):
        return ROLE_VENDOR
    ...
    # Important :
    # On ne retourne PAS manager/vendor/cashier ici,
    # car ces rôles ont besoin d'un profil staff.
    return None
